...
def _interactive_args(force=False):
    return argparse.Namespace(
        yes=False, force=force, skip_sync=False,
        dir=None, no_color=True, print_only=False,
    )
...
class TestYesGitLabModeInference:
    ...
    def test_no_tokens_infers_off(self, monkeypatch):
        assert setup._yes_gitlab_mode({}) == "off"

    def test_read_only_infers_read_only(self, monkeypatch):
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        assert setup._yes_gitlab_mode({}) == "read-only"

    def test_both_tokens_infers_read_write(self, monkeypatch):
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        monkeypatch.setenv("GITLAB_WRITE_TOKEN", "glpat-write")
        assert setup._yes_gitlab_mode({}) == "read-write"

    def test_explicit_env_wins_over_inferred(self, monkeypatch):
        monkeypatch.setenv("GITLAB_MODE", "off")
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        monkeypatch.setenv("GITLAB_WRITE_TOKEN", "glpat-write")
        assert setup._yes_gitlab_mode({}) == "off"

    def test_explicit_dotenv_wins_when_no_runtime_env(self):
...
class TestYesModeOff:
    ...
    def test_gitlab_mode_off_written_to_env(self, tmp_path, monkeypatch):
    ...
    def test_both_token_files_ensured_empty(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        setup.cmd_init(root, _yes_args(), Out(color=False))
    ...
    def test_no_projects_written_when_off_and_no_env_var(self, tmp_path, monkeypatch):
...
class TestYesModeReadOnly:
    ...
    def test_gitlab_mode_written(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        setup.cmd_init(root, _yes_args(), Out(color=False))
    ...
    def test_read_token_written(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        setup.cmd_init(root, _yes_args(), Out(color=False))
    ...
    def test_write_token_file_ensured_not_clobbered(self, tmp_path, monkeypatch):
    ...
    def test_warden_projects_from_env_written_to_toml(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
...
class TestYesModeReadWrite:
    ...
    def test_both_tokens_written(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        monkeypatch.setenv("GITLAB_READ_TOKEN", "glpat-read")
        monkeypatch.setenv("GITLAB_WRITE_TOKEN", "glpat-write")
...
class TestYesMigration:
    ...
    def test_stale_env_key_removed(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
    ...
    def test_stale_branch_prefix_removed(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        env_path = root / ".catraz" / ".env"
        env_path.write_text(
...
class TestInteractiveModeOff:
    ...
    def test_gitlab_mode_off_in_env(self, tmp_path, monkeypatch):
        root = self._run(tmp_path, monkeypatch)
        env = load_env(root / ".catraz" / ".env")
        assert env.get("GITLAB_MODE") == "off"

    def test_token_files_exist_without_getpass(self, tmp_path, monkeypatch):
        root = self._run(tmp_path, monkeypatch)
        secrets_dir = root / ".catraz" / "secrets"
        for fname in ("gitlab_read_token", "gitlab_write_token"):
            assert (secrets_dir / fname).exists()

    def test_warden_toml_unchanged(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
        original = (root / ".catraz" / "config" / "warden.toml").read_text()
...
class TestInteractiveModeReadOnly:
    ...
    def test_read_only_wizard(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
    ...
    def test_write_token_not_clobbered_when_existing(self, tmp_path, monkeypatch):
...
class TestInteractiveModeReadWrite:
    ...
    def test_read_write_wizard(self, tmp_path, monkeypatch):
        root = _make_root(tmp_path)
        _patch_common(monkeypatch)
    ...
    def test_enter_on_default_selects_read_write(self, tmp_path, monkeypatch):
...
class TestInteractiveAuthMode:
    ...
    def test_auth_mode_kept_when_present_and_no_force(self, tmp_path, monkeypatch):
...
class TestTomlSetters:
    ...
    def _copy_template(self, tmp_path):
        from catraz.paths import asset_root
        shipped = asset_root() / "assets" / "config" / "warden.toml"
    ...
    def test_set_toml_list_updates_allowed_projects(self, tmp_path):
    ...
    def test_set_toml_list_preserves_inline_comment(self, tmp_path):
    ...
    def test_set_toml_scalar_updates_branch_prefix(self, tmp_path):
        toml = self._copy_template(tmp_path)
        set_toml_scalar(toml, "branch_prefix", "feat/")
        text = toml.read_text()
        assert '"feat/"' in text

    def test_set_toml_scalar_preserves_comment(self, tmp_path):
    ...
    def test_set_toml_list_appends_when_key_absent(self, tmp_path):
        toml = tmp_path / "min.toml"
        toml.write_text('max_open_mrs = 5\n')
        set_toml_list(toml, "allowed_projects", ["group/proj"])
        assert _read_toml_allowed_projects(toml) == ["group/proj"]

    def test_set_toml_scalar_appends_when_key_absent(self, tmp_path):
        toml = tmp_path / "min.toml"
        toml.write_text('max_open_mrs = 5\n')
        set_toml_scalar(toml, "branch_prefix", "ci/")
        assert '"ci/"' in toml.read_text()

    def test_set_toml_list_matches_shipped_format(self, tmp_path):
    ...
    def test_set_toml_list_roundtrip_readable_by_function(self, tmp_path):
...
class TestUnsetEnvKeys:
    def test_removes_active_key(self, tmp_path):
    ...
    def test_leaves_comments_untouched(self, tmp_path):
        env_file = tmp_path / ".env"
        env_file.write_text(
    ...
    def test_removes_multiple_keys(self, tmp_path):
        env_file = tmp_path / ".env"
        env_file.write_text(
