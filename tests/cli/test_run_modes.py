...
def test_mode_parse_dispatches_oneoff(
    ...
    monkeypatch.setattr(
        run_cmd, "_run_oneoff", lambda root, out, sub, raw: captured.update(sub=sub, raw=raw) or 0
    )
...
def test_claude_remote_routes_to_daemon(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        run_cmd, "_start_remote_daemon", lambda root, args, out: called.update(hit=True) or 0
    )
    monkeypatch.setattr(
