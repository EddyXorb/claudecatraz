...
def _make_secrets(tmp_path, *, read_token="", write_token=""):
...
class TestGitLabModeOff:
    ...
    def test_check_gitlab_returns_ok(self):
        f = doctor.Findings()
    ...
    def test_check_tokens_probe_not_called(self, tmp_path, monkeypatch):
    ...
    def test_check_policy_no_bad_empty_allowlist(self, tmp_path):
...
class TestGitLabModeReadOnly:
    ...
    def test_read_token_set_no_bad(self, tmp_path, monkeypatch):
    ...
    def test_empty_read_token_is_bad(self, tmp_path):
        _make_secrets(tmp_path, read_token="", write_token="")
        f = doctor.Findings()
        doctor.check_tokens(tmp_path, {"GITLAB_MODE": "read-only"}, f)
    ...
    def test_write_token_present_warns(self, tmp_path, monkeypatch):
    ...
    def test_probe_only_read_token(self, tmp_path, monkeypatch):
        """In read-only mode only the read token is probed."""
        _make_secrets(tmp_path, read_token="glpat-readtoken", write_token="")
        probed = []

        def _fake_get(base, path, token, timeout=5):
            probed.append(token)
    ...
    def test_check_policy_nonempty_allowlist_ok(self, tmp_path, monkeypatch):
...
class TestGitLabModeReadWrite:
    ...
    def test_empty_read_token_bad(self, tmp_path):
    ...
    def test_empty_write_token_bad(self, tmp_path):
        _make_secrets(tmp_path, read_token="glpat-readtoken", write_token="")
        f = doctor.Findings()
        doctor.check_tokens(tmp_path, {"GITLAB_MODE": "read-write"}, f)
    ...
    def test_both_tokens_set_probes_both(self, tmp_path, monkeypatch):
        """In read-write mode, _probe_gitlab_tokens is called with both token pairs."""
        _make_secrets(tmp_path, read_token="glpat-readtoken",
                      write_token="glpat-writetoken")
        probe_calls = []

        def _fake_probe(root, env, f, tokens=None):
    ...
    def test_mode_absent_defaults_to_read_write(self, tmp_path):
...
class TestGitLabModeHelper:
    def test_absent_defaults_read_write(self):
        assert doctor._gitlab_mode({}) == "read-write"

    def test_empty_defaults_read_write(self):
...
def _mock_allowlist(tmp_path, monkeypatch, projects):
