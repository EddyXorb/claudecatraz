...
def test_tag_is_content_addressed(
    ...
    assert (
        seen["tag"].startswith("catraz-base:") and len(seen["tag"].split(":")[1]) == 12
    )
...
def test_base_context_overrides_build_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    _seed(
        tmp_path, "BASE_DOCKERFILE=./docker/Dockerfile.base\nBASE_CONTEXT=./ctxroot\n"
    )
    seen: dict[str, Any] = {}
