...
class TestAsk:
    def test_returns_typed_value(self, out, monkeypatch):
        monkeypatch.setattr("builtins.input", lambda prompt: "typed")
        assert out.ask("Enter value", default="fallback") == "typed"

    def test_returns_default_on_empty_input(self, out, monkeypatch):
        monkeypatch.setattr("builtins.input", lambda prompt: "")
        assert out.ask("Enter value", default="fallback") == "fallback"

    def test_returns_default_on_eof(self, out, monkeypatch):
        def raise_eof(prompt):
            raise EOFError
        monkeypatch.setattr("builtins.input", raise_eof)
        assert out.ask("Enter value", default="fallback") == "fallback"

    def test_returns_empty_string_when_no_default_and_empty_input(self, out, monkeypatch):
        monkeypatch.setattr("builtins.input", lambda prompt: "")
        assert out.ask("Enter value") == ""

    def test_returns_empty_string_when_no_default_and_eof(self, out, monkeypatch):
        def raise_eof(prompt):
            raise EOFError
        monkeypatch.setattr("builtins.input", raise_eof)
        assert out.ask("Enter value") == ""
...
class TestChoice:
    OPTIONS = [("off", "No GitLab"), ("read-only", "Read only"), ("read-write", "Read + write")]
    ...
    def test_returns_selected_value_for_valid_number(self, out, monkeypatch):
        monkeypatch.setattr("builtins.input", lambda prompt: "3")
        result = out.choice("Pick mode", self.OPTIONS, default=0)
        assert result == "read-write"

    def test_terminates_and_returns_default_after_3_junk_inputs(self, out, monkeypatch):
        """Proves the retry loop is bounded: constant non-empty junk never loops forever."""
        call_count = 0

        def always_junk(prompt):
            nonlocal call_count
            call_count += 1
            return "garbage"
...
class TestSecret:
    def test_returns_typed_secret(self, out, monkeypatch):
        monkeypatch.setattr("getpass.getpass", lambda prompt: "s3cr3t")
        assert out.secret("Token") == "s3cr3t"

    def test_returns_current_on_empty_input(self, out, monkeypatch):
        monkeypatch.setattr("getpass.getpass", lambda prompt: "")
        assert out.secret("Token", current="existing") == "existing"

    def test_returns_current_on_eof(self, out, monkeypatch):
        def raise_eof(prompt):
            raise EOFError
        monkeypatch.setattr("getpass.getpass", raise_eof)
        assert out.secret("Token", current="existing") == "existing"

    def test_returns_empty_when_no_current_and_empty_input(self, out, monkeypatch):
        monkeypatch.setattr("getpass.getpass", lambda prompt: "")
        assert out.secret("Token") == ""

    def test_returns_typed_value_over_current(self, out, monkeypatch):
