...
class TestAsk:
    ...
    def test_returns_default_on_empty_input(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_empty_string_when_no_default_and_empty_input(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("builtins.input", lambda prompt: "")
        assert out.ask("Enter value") == ""

    def test_returns_empty_string_when_no_default_and_eof(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
        def raise_eof(prompt: str) -> None:
            raise EOFError
        monkeypatch.setattr("builtins.input", raise_eof)
        assert out.ask("Enter value") == ""
...
class TestChoice:
    ...
    def test_returns_selected_value_for_valid_number(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("builtins.input", lambda prompt: "3")
        result = out.choice("Pick mode", self.OPTIONS, default=0)
        assert result == "read-write"

    def test_terminates_and_returns_default_after_3_junk_inputs(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestSecret:
    ...
    def test_returns_current_on_empty_input(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_returns_empty_when_no_current_and_empty_input(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("getpass.getpass", lambda prompt: "")
        assert out.secret("Token") == ""

    def test_returns_typed_value_over_current(self, out: Out, monkeypatch: pytest.MonkeyPatch) -> None:
