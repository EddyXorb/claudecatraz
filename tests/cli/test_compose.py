...
def test_base_cmd_sets_unique_project_name(tmp_path):
    cmd = compose.base_cmd(tmp_path)
    assert "--project-name" in cmd
    assert cmd[cmd.index("--project-name") + 1] == compose.project_name(tmp_path)


def test_project_name_is_valid_stable_and_unique(tmp_path):
    a = tmp_path / "work" / "api"
...
def test_project_name_handles_exotic_basename(tmp_path):
