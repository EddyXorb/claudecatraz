...
def _resolve_host_ipv4(host: str) -> str | None:
    """Real upstream IPv4 for a git-endpoint host, resolved on the host machine —
    which is not on agent-net, so it sees the true address, not the warden's own
    alias. Best-effort: None when resolution fails, leaving DNS to Docker."""
    try:
...
def render_hosts_fragment(hosts: list[str], host_ips: dict[str, str] | None = None) -> str:
    ...
    aren't swallowed by the forward proxy instead. For each host in *host_ips*,
    also pin the real IP into the warden's own /etc/hosts (see below)."""
    host_ips = host_ips or {}
    # One entry per host: a duplicate [[git.endpoint]] host would emit duplicate
    # aliases/extra_hosts, which Docker rejects. Order-preserving dedup.
    ...
    if hosts:
        ...
        lines.append("        aliases: []")
    # The warden owns each host's agent-net alias, so Docker DNS resolves the host
    # to the warden itself — its own upstream reconcile/proxy calls would loop back
    # and fail. Pinning the real IP in the warden's /etc/hosts (consulted before
    # Docker DNS) redirects only the warden's own lookups to the true host; the
    # agent still resolves the host to the alias and stays routed through the warden.
    pinned = [(host, host_ips[host]) for host in hosts if host in host_ips]
    if pinned:
        lines.append("    extra_hosts:")
        lines += [f'      - "{host}:{ip}"' for host, ip in pinned]
    lines += [
        "  claude-dev-env:",
        "    environment:",
...
def write_hosts_fragment(root: Path) -> Path:
    ...
    list and return its path, so it never drifts from warden.toml. Also resolves
    each host's real IP (best-effort) to pin it into the warden's extra_hosts, so
    the warden's own lookups don't loop back through its alias. Safe to call
    unconditionally; a host that fails to resolve is simply left unpinned."""
    path = _hosts_fragment_path(root)
    hosts = _git_endpoint_hosts(root)
    host_ips = {host: ip for host in hosts if (ip := _resolve_host_ipv4(host)) is not None}
    path.write_text(render_hosts_fragment(hosts, host_ips))
    return path
