...
def _git_endpoint_hosts(root: Path) -> list[str]:
    """``[[git.endpoint]].host`` entries from ``.catraz/config/warden.toml``,
    best-effort (§07 — compose-and-agent-routing).

    Bare host strings only — this is all the generated per-host compose
    fragment (:func:`render_hosts_fragment`) needs (DNS alias + no_proxy), a
    different shape from ``doctor._read_git_endpoints`` (which also carries
    ``type`` for its token cross-checks) kept deliberately separate rather
    than shared: ``doctor`` imports this module already (:func:`run`), so the
    reverse import would be circular, and the two callers want different
    fields from the same TOML anyway. Absent/unreadable/malformed file or
    array all yield ``[]`` — the Warden's own loader is the fail-closed side
    (``warden/warden/core/config_load.py``); compose only ever renders
    best-effort convenience wiring on top.
    """
...
def render_hosts_fragment(hosts: list[str]) -> str:
    """The compose override fragment that makes every configured git-endpoint
    host actually reachable (§1.1/§1.2, §07 compose-and-agent-routing):

    - a DNS alias on the warden's own ``agent-net`` membership per host (so
      ``<host>`` resolves to the Warden container from inside the isolated
      agent network — no second Warden, no per-host guard, one alias each);
    - the same hosts (plus loopback) in the agent's ``no_proxy``/``NO_PROXY``,
      so REST calls meant for the Warden aren't swallowed by the forward
      proxy instead.

    Deliberately generic: the same rule for N hosts, no per-host guard/warden
    duplication, and the Warden's container name never appears here — hosts
    are exactly what ``warden.toml`` lists, verbatim.
    """
...
def write_hosts_fragment(root: Path) -> Path:
    """(Re)generate ``.catraz/compose.hosts.yml`` from the current
    ``[[git.endpoint]]`` list and return its path — called by
    :func:`generate_resolved` (the render=True path: up/run/shell/down) so it
    never drifts from ``warden.toml``. Cheap, pure-Python, no subprocess —
    safe to call unconditionally whenever a real render happens."""
...
def _source_cmd(root: Path) -> list[str]:
    ...
    # Per-host DNS alias/no_proxy fragment (§07) — generated by write_hosts_fragment
    # (called from generate_resolved, the render=True path). Included only when it
    # already exists (same existence gate as compose.override.yml below): before the
    # first `up`/`run` there is nothing running yet, so an absent fragment is a
    # no-op, not an error — _source_cmd itself never writes files (read-only callers
    # like status/logs must stay side-effect-free).
...
def generate_resolved(root: Path, extra_env: dict[str, str] | None = None) -> bool:
    ...
    """
    write_hosts_fragment(root)  # keep the per-host DNS-alias/no_proxy layer fresh (§07)
