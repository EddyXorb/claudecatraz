...
Every :class:`~warden.model.Decision` carries a bare rule id ("R0".."R6") for
the audit log. Before this module those ids were string literals scattered
across ``policy.py``, ``api_endpoints.py``, ``read_endpoints.py``,
``git_proxy.py`` and ``api_proxy.py`` (Befund B3/F5) — one place could drift
from another with no compiler check. This module is the *one* definition:
callers import :data:`R0`..:data:`R6` instead of writing string literals, and
:func:`rule` is the single lookup that resolves a bare id back to its
definition (meta-rule + human summary), so a typo'd or unregistered id fails
loudly instead of silently reaching the audit log.
