"""Env + warden.toml → :class:`~warden.config.Config` (W10).

The loading half of the config layer: :mod:`warden.config` holds the pure,
frozen :class:`~warden.config.Config` value (what the policy consumes); this
module holds everything that *produces* one — secret files, TOML parsing,
...
def _parse_endpoint_activation(file: Mapping[str, object]) -> "EndpointActivation":
    ...
    ``config._default_endpoint_activation`` for why ``catalog`` is never
    imported at module scope here. Malformed shape ⇒ ``ConfigError``
    (fail-closed startup abort), the same treatment as a malformed
    ``warden.toml`` overall.
    """
    from .catalog.config_parse import EndpointConfigError, parse_endpoint_activation
