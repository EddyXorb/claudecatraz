...
class FieldSpec:
    ...
    F12 fix: ``api_proxy`` extracts *only* the fields an entry declares here,
    each from its declared location — never a blind body+query merge. A field
    declared ``BODY`` that only shows up in the query string (or vice versa)
    is simply absent from the decision, exactly as if the caller never sent
    it — closing the footgun where a scoping check could "pass" on a value
    the upstream request never actually carried.
    """
...
CheckFn = Callable[[ProxyRequest, StateView, Config], Optional[Decision]]
...
class RegisteredCheck:
    ...
    an ownership check — so a caller (``api_proxy._resolve_ownership``) can
    decide whether to run an unpure lookup *by declared need*, not by
    ...
    def __call__(self, req: ProxyRequest, state: StateView, cfg: Config) -> Optional[Decision]:
        return self.fn(req, state, cfg)
...
class DenyProbe:
    ...
    startgate builds a :class:`~warden.model.ProxyRequest` directly (no HTTP
    parsing involved), so it does not need to distinguish location: it sets
    ``req.fields`` verbatim. ``mr_owner_ok`` covers probes for
    ownership-gated entries, where the datum being probed is not a wire field
    at all but the out-of-band ownership lookup (``None`` — unverifiable — is
    itself already a natural deny case for those entries).
    """
