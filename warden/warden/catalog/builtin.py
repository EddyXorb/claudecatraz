...
``policy._decide_api`` checks it before ever consulting the effective table.

The FORBIDDEN capability layer (``capabilities.py``) is a second, independent
reason this endpoint can never be allowed even if this match were somehow
bypassed (defense-in-depth, A10) — see ``test_capabilities.py``'s proof that a
hypothetical catalog row shaped like this one is still denied by the
capability layer alone.
"""
