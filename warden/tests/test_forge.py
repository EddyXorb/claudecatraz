...
# the numeric-id alias is forge state now, resolved by reconcile, not a Config
# concern — see forge.reconcile's docstring) -----------------------------------
def test_project_allowed_by_id_matches_reconciled_alias_only(cfg):
    forge = _forge(cfg)
    forge.project_id_aliases = {"81882161"}
    assert forge.project_allowed_by_id("81882161")
    assert not forge.project_allowed_by_id("99999999")  # unknown id: default-deny


# --- pagination (the bugfix) ---------------------------------------------------
async def test_list_branches_follows_every_page(forge, respx_router):
...
async def test_reconcile_populates_counters_and_unlocks(cfg, respx_router):
    ...
    # The numeric-id alias was resolved and added to the forge's alias set
    # (R6 by id form) — Config itself is never mutated (D2).
    assert forge.project_id_aliases == {"12345"}
    assert forge.project_allowed_by_id("12345")
    await forge.upstream.aclose()
