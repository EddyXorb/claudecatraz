"""ownership.py (W6.2, §07 Punkt 4/6): the REST-API guard's MR
...
def _ownership(cfg, *, clock=None) -> MrOwnership:
    ...
    return MrOwnership(Upstream(cfg), cfg, **kwargs)
...
async def test_source_in_namespace_true_when_prefixed_regardless_of_author(cfg, respx_router):
    ...
    assert await ownership.source_in_namespace("group/proj", 7) is True
...
async def test_source_in_namespace_false_when_prefix_missing(cfg, respx_router):
    ...
    assert await ownership.source_in_namespace("group/proj", 7) is False
...
async def test_source_in_namespace_none_when_lookup_fails(cfg, respx_router):
    ...
    assert await ownership.source_in_namespace("group/proj", 7) is None
